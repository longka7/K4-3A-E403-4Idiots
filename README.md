...
|---|---|---|---|
| Nguyễn Long Khánh | 2A202602649 | Leader + BA | **CP1:** Chốt Canvas 4 ô, lát cắt 1 câu, tạo repo, phân công vai trò ban đầu cho 3 thành viên. **CP2:** Dựng bản mock bấm được đầu tiên (`codebase/mock-cp2.html`) và sơ đồ luồng (`codebase/flow-diagram.md`). **CP1-CP4:** Viết `spec.md` §1-§2 (bằng chứng khảo sát n=23, bảng impact 4 ứng viên), điều phối tiến độ, xử lý đổi vai giữa An/Thế Anh khi cần, tổng hợp và gộp `spec.md` từ cả 3 thành viên (§4b, §5, §6, §7) thành bản thống nhất, xử lý xung đột khi merge code. **Từ CP4:** Rà soát và nâng cấp toàn bộ giao diện `codebase/mock-cp2.html` — đồng bộ màu sắc/font, tối ưu bố cục luồng, hoàn thiện các thông báo trạng thái/lỗi. Nộp đủ các mốc CP bằng đúng một mã học viên. |
| Trần Thế Anh | 2A202602516 | AI/Prototype Engineer → Data/Eval Analyst (đổi vai từ CP3) | **CP3:** Xây bộ golden set 20 case (`eval/golden-set.csv`) đúng cơ cấu bắt buộc — 8 case khó (2 case/lớp chỗ khó), 8 case thường, 4 case hiếm; 12/20 case phát triển và verify từ chatlog thật của khoá trước. **CP4:** Viết `spec.md` §5 — cụ thể hoá 4 lớp chỗ khó theo lát cắt D2 và ≥8 kịch bản rủi ro (mỗi kịch bản: tình huống · lớp · hành vi mong muốn · nguyên tắc áp dụng), dựng trực tiếp từ 8 case khó đã xây. |
| Nguyễn Văn An | 2A202602776 | Data/Eval Analyst → AI/Prototype Engineer (đổi vai từ CP3) | **CP3:** Nhận lại phần AI/Prototype — nối AI thật (Gemini) vào hàm chẩn đoán lỗi `showDiagnosis()`, viết system prompt ép đúng 3 quy tắc (chỉ dùng tài liệu được cấp, nói rõ "chưa xác định được" khi thiếu căn cứ, không lộ đáp án), test với golden set. **CP4:** Phát hiện và tự sửa lỗi trong script chấm điểm tự động (đang đếm độ dài câu trả lời thay vì đối chiếu nội dung), chạy lại toàn bộ 20 case lấy số liệu thật kèm phân tích nguyên nhân case chưa đạt, viết `spec.md` §6 (4 đường đi trải nghiệm dùng log thật). Thiết kế lại giao diện bộ trắc nghiệm đầu vào (Bước 0 — 5 câu, có gợi ý và giải thích đáp án). |
| Nguyễn Tuấn Khanh | 2A202602819 | UX/Validation Lead | **CP3:** Viết kịch bản quay video thao tác 30 giây theo đúng luồng AI thật. **CP4:** Viết `spec.md` §4b — chọn 4 nguyên tắc HAX/PAIR (G2, G4, G8, G9) và chỉ rõ vị trí áp dụng cụ thể trong `mock-cp2.html`. **CP5:** Tìm ≥5 người ngoài nhóm dùng thử thật (≥2 người đã khai từ CP1: Nguyễn Duy Khánh, Lưu Xuân Dũng), ghi log + quote nguyên văn vào `validation/`, quay video demo dự phòng. |
